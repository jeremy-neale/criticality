...
export const GLOSSARY = {
  ...
  aura:     { name: 'Aura', text: '±X% damage for 4 rounds. Outgoing auras change the damage you deal; incoming auras change the damage you take. Each player has ONE aura slot — a new aura replaces the old one, good or bad.' },
  brace:    { name: 'Brace', text: '\u2212X% incoming damage for 4 rounds. Brace is an aura, so any new aura replaces it. Pierceable.' },
...
export const RULES_SUMMARY = [
  ...
  `Each turn: click a card, then Cast it or Discard it. Discarding is free — ditch as many cards as you like and still cast or pass afterwards. Casting or passing ends your turn, and your hand refills to 7. ${RULES.turnSecs}s per turn.`,
  `Damage = base x (1 + damage% + blades) x aura x bubble x (1 - weakness) x (1 - max(0, resist - pierce)) x shields/brace (each pierceable) x (1 + traps on the target). Auras and brace share one slot per player.`,
  ...
  `Auras last ${RULES.auraRounds} rounds. Each player has one aura slot: a new aura replaces the old one instead of stacking.`,
